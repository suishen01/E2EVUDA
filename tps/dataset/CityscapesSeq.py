...
class CityscapesSeqDataSet(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        # previous
        name_kf = name_cf.replace(str(frame_cf).zfill(6) + '_leftImg8bit.png', str(frame_cf - 1 - self.interval).zfill(6) + '_leftImg8bit.png')
        file_kf = self.root / 'leftImg8bit_sequence' / self.set / name_kf
        ...
        if self.set == 'train':
            return image_d.copy(), label.copy(), image_c.copy(), label.copy(), image_b.copy(), label.copy(), np.array(image_d.shape), name_cf, d.transpose((2, 0, 1)), c.transpose((2, 0, 1)), b.transpose((2, 0, 1))
