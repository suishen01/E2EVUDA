...
class BaseDataset(data.Dataset):
    def __init__(self, root, list_path, set_,
        ...
        if self.set == 'val':
            self.list_path = self.list_path.replace('train.txt', 'val.txt')
    ...
    def get_labels_synthia_seq(self, file):
        '''dir = '../../../TPSold/data/SynthiaSeq/SEQS-04-DAWN/label/'
        output_dir = '../../../TPSold/data/SynthiaSeq/SEQS-04-DAWN/label_new/'
        ...
        for file in files:
            ...
            raw_label_p = img[:, :, -1]
            print(raw_label_p.shape)
            print(np.unique(raw_label_p))
        jomi'''
        # img = Image.open(file)
        img = cv2.imread(str(file),-1)
        raw_label_p = img[:, :, -1]
        img = Image.fromarray(raw_label_p)
        img = img.resize(self.labels_size, Image.NEAREST)
        return np.asarray(img, np.float32)
